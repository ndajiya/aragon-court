...
contract('Hex Sum Tree', (accounts) => {
  ...
  it('inserts two', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      assertBN(await tree.sortition(i, 0), 0, `Draw first, value ${i}`)
    }
    for (let i = 5; i < 10; i++) {
      //await logSortition(i)
      assertBN(await tree.sortition(i, 0), 1, `Draw second, value ${i}`)
    }
  })
  ...
  it('sortition', async () => {
    ...
    assertBN(await tree.sortition(1, 0), 0)
    assertBN(await tree.sortition(11, 0), 1)
    assertBN(await tree.sortition(171, 0), 17)
  })
  ...
  it('inserts into another node', async () => {
    ...
    assertBN(await tree.sortition(2605, 0), 260)
  })
})
